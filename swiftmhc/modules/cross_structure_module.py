...
from openfold.model.structure_module import StructureModuleTransition
from openfold.np.residue_constants import restype_atom14_mask
from openfold.np.residue_constants import restype_atom14_rigid_group_positions
from openfold.np.residue_constants import restype_atom14_to_rigid_group
from openfold.np.residue_constants import restype_rigid_group_default_frame
from openfold.utils.feats import frames_and_literature_positions_to_atom14_pos
from openfold.utils.feats import torsion_angles_to_frames
...
class CrossStructureModule(torch.nn.Module):
    ...
    def __init__(self, config: ml_collections.ConfigDict):
        ...
        self.cross_ipa_transition = StructureModuleTransition(
            self.c_s, self.n_transition_layers, self.dropout_rate
        )
                    ...
                    for row_index in range(attention_weights.shape[-2]):
                        f.write(
                            " ".join(
                                [
                                    str(x.item())
                                    for x in attention_weights[batch_index, head_index, row_index]
                                ]
                            )
                            + "\n"
                        )
    ...
    def _block(
        ...
        backb_to_global = Rigid(
            Rotation(rot_mats=T_peptide.get_rots().get_rot_mats(), quats=None),
            T_peptide.get_trans(),
        )
        ...
        unnormalized_angles = torch.cat(
            [omegas.unsqueeze(-2), unnormalized_angles[..., 1:, :]], dim=-2
        )
    ...
    def _calculate_omegas_from_positions(self, positions: torch.Tensor, res_mask: torch.Tensor):
        ...
        # [*, N_res - 1, 3]
        newmann0 = torch.nn.functional.normalize(
            vec_CCA0 - (plane_n * vec_CCA0).sum(dim=-1).unsqueeze(-1) * plane_n, dim=-1
        )

        # [*, N_res - 1, 3]
        newmann1 = torch.nn.functional.normalize(
            vec_NCA1 - (plane_n * vec_NCA1).sum(dim=-1).unsqueeze(-1) * plane_n, dim=-1
        )
