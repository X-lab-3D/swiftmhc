...
def _load_bond_definitions() -> dict[str, tuple[str, str]]:
    ...
    for line in stereo_chemical_props.splitlines():
        ...
        bonds_per_amino_acid[amino_acid_code] = bonds_per_amino_acid.get(amino_acid_code, []) + [
...
residue_atom_sets = {
    residue_name: set(atom_names) for residue_name, atom_names in residue_atoms.items()
}
...
def build_modeller(
    ...
    for chain_id, residue_numbers, aatype, atom14_positions, atom14_mask in chain_data:
        ...
        for residue_index, amino_acid_index in enumerate(aatype):
            ...
            replacement_amino_acid_index, replacement_atom14_mask = (
                _replace_amino_acid_with_missing_atoms(amino_acid_index, atom14_mask[residue_index])
            )

            amino_acid_code = amino_acid_order[amino_acid_index]
            residue = topology.addResidue(
                amino_acid_code, chain, str(residue_numbers[residue_index].item())
            )
            ...
            for atom_index, atom_name in enumerate(restype_name_to_atom14_names[amino_acid_code]):
                if replacement_atom14_mask[atom_index] and len(atom_name) > 0:
                    ...
                    atom = topology.addAtom(
                        atom_name, Element.getBySymbol(atom_name[0]), residue, str(atom_nr)
                    )
            ...
            if "CA" in positions_by_name and "N" in positions_by_name and "O" in positions_by_name:
                ...
                if "CB" in positions_by_name:
                    ...
                    h_direction = torch.nn.functional.normalize(
                        torch.nn.functional.normalize(c_ca, dim=-1)
                        + torch.nn.functional.normalize(n_ca, dim=-1)
                        + torch.nn.functional.normalize(cb_ca, dim=-1),
                        dim=-1,
                    )
                    if torch.any(h_direction == torch.nan):
                        raise RuntimeError("cannot determine the location of the alpha hydrogen")
