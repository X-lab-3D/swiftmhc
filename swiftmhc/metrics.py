...
class MetricsRecord:
    ...
    def _store_individual_affinities(self, pass_name: str, directory_path: str):
        ...
        # save to file
        table.to_csv(
            affinities_path, sep=",", encoding="utf-8", index=False, quoting=csv.QUOTE_NONNUMERIC
        )
    ...
    def _store_metrics_table(self, epoch_number: int, pass_name: str, directory_path: str):
        ...
        for loss_name, loss_value in self._losses_sum.items():
            ...
            table.loc[row_mask, f"{pass_name} {loss_name} loss"] = normalized

        # write rmsd
        mean = round(numpy.mean(list(self._rmsds.values())), 3)
        table.loc[row_mask, f"{pass_name} mean binders C-alpha RMSD(Å)"] = mean

        # write affinity-related metrics
        if (
            "class" in self._output_data
            and "class" in self._truth_data
            and self._has_distribution(self._truth_data["class"])
        ):
            self._store_classification(table, row_mask, pass_name)

        if (
            "affinity" in self._output_data
            and "affinity" in self._truth_data
            and self._has_distribution(self._truth_data["class"])
        ):
        ...
        # store metrics
        table.to_csv(
            metrics_path, sep=",", encoding="utf-8", index=False, quoting=csv.QUOTE_NONNUMERIC
        )
