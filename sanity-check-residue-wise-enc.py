...
from tcrspec.modules.transform import DebuggableTransformerEncoderLayer
...
class Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.enc = DebuggableTransformerEncoderLayer(mlp_input_size, 1)
        ...
        self.aff_mlp = torch.nn.Sequential(
            torch.nn.Linear(9, c_aff),
            torch.nn.GELU(),
            torch.nn.Linear(c_aff, 1)
        )
    ...
    def forward(self, seq_embd: torch.Tensor) -> torch.Tensor:
        ...
        pos_enc = torch.eye(loop_len, 32).unsqueeze(0).expand(seq_embd.shape)

        seq_embd = torch.cat((seq_embd, pos_enc), dim=2)

        seq_mask = torch.ones(batch_size, loop_len, dtype=torch.bool)
        seq_embd, att = self.enc(seq_embd, seq_mask)
