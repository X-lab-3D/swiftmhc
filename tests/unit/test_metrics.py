...
def test_metrics():
    ...
    mean_loss = round(
        sum([loss["total"] * batch_size for loss in loss_datas]).item() / (batch_size * n_batch), 3
    )
    ...
    try:
        for epoch_index in range(n_epoch):
            ...
            for batch_index in range(n_batch):
                record.add_batch(
                    loss_datas[batch_index], pred_datas[batch_index], batch_datas[batch_index]
                )
    ...
    for key in (
        f"{pass_name} mean binders C-alpha RMSD(Å)",
        f"{pass_name} accuracy",
        f"{pass_name} total loss",
    ):
