...
def test_modeller():
    ...
    atom14_mask = torch.cat(
        (protein["atom14_gt_exists"], torch.zeros(padding, 14, dtype=torch.bool)), dim=0
    )
    atom14_positions = torch.cat(
        (protein["atom14_gt_positions"], torch.zeros(padding, 14, 3)), dim=0
    )
