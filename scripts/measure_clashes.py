...
arg_parser = ArgumentParser(
    description="Given a series of PDB chains, list all clashes between that chain and"
...
def successive_residues(residue1: Residue, residue2: Residue) -> bool:
    ...
    if n1 == n2:
        ...
        if len(i1.strip()) > 0 and len(i2.strip()) > 0:
            ...
            raise ValueError(
                f"two residues with number {n1} in chain {residue1.get_parent().get_id()} in structure {residue1.get_parent().get_parent().get_id()}"
            )
...
def must_ignore_clash(atom1: Atom, atom2: Atom) -> bool:
    ...
    if element1 == "H" or element2 == "H":
        ...
        if is_backbone(atom1) and is_backbone(atom2):
            ...
            residue1.get_resname() == "PRO"
            and is_backbone(atom2)
            or residue2.get_resname() == "PRO"
            and is_backbone(atom1)
        ):
    ...
    for i, atom1 in enumerate(peptide_atoms):
        ...
        for atom2 in other_atoms + peptide_atoms[i + 1 :]:
            ...
            if distance < (radius1 + radius2):
                _log.debug(
                    f"found a clash between {atom1.get_parent()} {atom1} and {atom2.get_parent()} {atom2}"
                )
