...
arg_parser.add_argument(
    "--align-ref-pdb", "-r", help="path to a pdb structure to align the structures in the video to"
)
...
def get_rmsd(struct1: Structure, struct2: Structure) -> float:
    ...
    if len(residues1) != len(residues2):
        raise ValueError(
            f"not the same length: {struct1.id} chain P has {len(residues1)} residues, while {struct2.id} chain P has {len(residues2)}"
        )
...
if __name__ == "__main__":
    ...
    with h5py.File(args.hdf5_path, "r") as hdf5_file:
        true_structure_s = "".join(
            [b.decode("utf-8") for b in hdf5_file["true/structure"][:].tolist()]
        )
        ...
        for frame_index, frame_id in enumerate(frame_ids):
            ...
            snapshot_structure_s = "".join(
                [b.decode("utf-8") for b in hdf5_file[f"{frame_id}/structure"][:].tolist()]
            )
